  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (result?.result === 'success' || result?.success) {
        ...
        Alert.alert(
          t('common.error'),
          result?.error || t('auth.email_otp.verify_error'),
        );
      }
    } catch (error: any) {
      Alert.alert(
        t('common.error'),
        getApiErrorMessage(error, t('auth.email_otp.verify_error')),
      );
    } finally {
      setLoading(false);
    }
